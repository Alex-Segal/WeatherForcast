Introduction:
First, I would like to thank for your time checking my assigment.
...
Since it take longitude and latitude as input, I used https://geocode.maps.co/ to convert string address to coordinates.
...
3. Util folder contain several modules that perform different tasks like communication with APIs and other helper classes.
4. For cache storage I use a Cache with expireAfterWrite option to make it time oriented. I have never used it before, but I found it online, and it was easy to learn how to use it.
   The Storage using singleton design pattern and synchronized to be thread safe. We have only 1 user at the time but why not to thing into the future...
5. A configuration file is used to store constants like API_KEYs URLs etc.
6. https://geocode.maps.co/ API try to guess the address even when random characters are provided. It also gives back different json result based on input.
   I didn't check all possible inputs. I tested a full address with and without zip and bad entry address. I made some null protections, but there is plenty that can be added.
   Many hours can be spent just testing the result jsons it provides with different inputs and protecting it from nullPointerExceptions.